...
@section('content')
	...
	<ul class="nav nav-tabs">
		<li class="active"><a href="#post" data-toggle="tab">Tạo bài viết</a></li>
		<li><a href="#slideshow" data-toggle="tab">Tạo Slideshow</a></li>
	</ul>
		...
		<div id="post" class="tab-pane fade in active">
			<div class="panel-body">
				...
				<div class="form-group">
					...
					<textarea class="form-control" type="text" id="description" name="descriptions"></textarea>
				</div>
				<div class="form-group avatar">
					<label for="avatar">Ảnh đại diện:</label>
					<input name="image" type="text" id="image" class="form-control" value="{{ URL::asset('img/no-image.jpg') }}" onblur="$(this).parents('.avatar').find('img').attr('src', $(this).val());$(this).parents('.avatar').find('img').attr('value', $(this).val())">
					<p style="margin-top:10px">
						<button class="btn btn-info" onclick="$('#upload').modal('show'); value_element = '.avatar'">Chọn ảnh </button>
						<img class="img-thumbnail" src="{{ URL::asset('img/no-image.jpg') }}" value="{{ URL::asset('img/no-image.jpg') }}" style="width: 120px;">
					</p>
				</div>
				<div class="form-group">
				...
				<div class="form-group">
					...
					<textarea name="content_" id="content_" cols="30" rows="10"></textarea>
				</div>
			</div>
    ...
    <script type="text/javascript">
    	...
    	function sendForm() 
    	{
    		...
    		$.ajax({
    			type: 'post',
    			dataType: 'json',
    			url: '{{ route('admin.service.tfe.store') }}',
    			data: {
					'_token' : '{{ csrf_token() }}',
					'name' : $("input#name").val(),
    	...
    	function addImage()
    	{
			...
			html += "	<input type=\"hidden\" value=\"" + number + "\">"
			html +=	"<\/th>";
			html += "<\/tr>";
			$("tbody").append(html);
			number++;
